...
class CrystalToolKitDisplay:
    ...
    def __init__(self, atoms:Atoms, with_bonds=None, bond_nn_class=None, scene_kwargs=None, legend_kwargs=None):
        """Instantiate the object"""

        from pymatgen.analysis.local_env import MinimumDistanceNN
        from pymatgen.io.ase import AseAtomsAdaptor

        bond_nn_class = bond_nn_class if bond_nn_class is not None else DISPLAY_SETTINGS.bond_nn_class

        if bond_nn_class is None:
            self.bond_nn_class = MinimumDistanceNN
    

        self.scene_kwargs = scene_kwargs if scene_kwargs is not None else DISPLAY_SETTINGS.scene_kwargs
        self.legend_kwargs = legend_kwargs if legend_kwargs is not None else  DISPLAY_SETTINGS.legend_kwargs

        self.atoms = atoms
        self.ps = AseAtomsAdaptor.get_structure(atoms)
        self.with_bonds = with_bonds if with_bonds is not None else DISPLAY_SETTINGS.with_bonds
    ...
    def build_scene(self):
        ...
        if self.with_bonds:
            ...
            graph = StructureGraph.with_local_env_strategy(self.ps, self.bond_nn_class())
